...
const statValues = ["500+", "100%", "10+", "5.0"];
const statColors = ["text-bubble-primary", "text-green-600", "text-yellow-500", "text-purple-600"];
...
export default function TestimonialsClient() {
    ...
    const categoryLabels: Record<Category, string> = {
        all: t("filterAll"),
        installation: t("filterInstallation"),
        repair: t("filterRepair"),
        replacement: t("filterReplacement"),
    };

    const statLabels = [t("stat1Label"), t("stat2Label"), t("stat3Label"), t("stat4Label")];
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-8 text-center">
                        {statValues.map((val, i) => (
                            <AnimatedSection key={i} from="bottom" delay={i * 80}>
                                <div className={`text-4xl font-extrabold ${statColors[i]} mb-1`}>{val}</div>
                                <div className="text-sm text-gray-500 font-medium">{statLabels[i]}</div>
                            </AnimatedSection>
                        ))}
                    </div>
                </div>
            ...
            <section className="py-20 bg-gray-50">
                <div className="container mx-auto px-4 sm:px-6 lg:px-8 max-w-7xl">
                    <div className="text-center mb-12">
                        ...
                        <p className="text-xl text-gray-600 max-w-3xl mx-auto mb-8">{t("reviewsSubtitle")}</p>
                        {/* Filters */}
                    ...
                    <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
                        {filtered.map((r, i) => (
                            <AnimatedSection key={r.name} from="bottom" delay={i * 60} className={r.featured ? "md:col-span-2 lg:col-span-3" : ""}>
                                <div className={`bg-white rounded-2xl shadow-sm border border-gray-100 p-6 hover:shadow-xl transition-all flex flex-col h-full ${r.featured ? "border-bubble-primary/20" : ""}`}>
                                    ...
                                    <span className={`text-xs font-semibold px-3 py-1 rounded-full w-max ${categoryColorMap[r.category]}`}>
                ...
                <div className="container mx-auto px-4 max-w-4xl relative z-10">
                    <AnimatedSection from="bottom">
                        ...
                        <div className="flex flex-col sm:flex-row justify-center gap-4">
                            <Link href={lp("/contact")} className="inline-flex items-center justify-center gap-2 bg-white text-bubble-primary px-10 py-5 rounded-full font-bold text-lg hover:bg-gray-100 transition shadow-xl">
