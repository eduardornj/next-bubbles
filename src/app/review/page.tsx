...
const PLATFORMS = [
  {
    ...
    description: "Help us show up when homeowners search for soffit in Orlando",
  ...
  },
  {
    ...
    description: "Yelp reviews help homeowners compare local contractors",
    icon: (
  ...
  },
  {
    ...
    description: "Your recommendation reaches friends, family, and neighbors",
    icon: (
  ...
  },
  {
    ...
    description: "Your neighbors trust recommendations from people they know",
    icon: (
  ...
  },
  {
    ...
    description: "Angi is where homeowners find top-rated contractors",
    icon: (
    ...
    <div className="flex flex-col min-h-screen">
      ...
      <section className="py-24 bg-gradient-to-br from-slate-900 via-bubble-navy to-bubble-dark text-white relative overflow-hidden">
        ...
        <div className="container mx-auto px-4 sm:px-6 lg:px-8 text-center max-w-3xl relative z-10">
          ...
          <p className="text-xl text-blue-100 leading-relaxed max-w-2xl mx-auto">
            Every platform matters. Each review on a different site reaches a different audience. Can you leave one on all five?
          </p>
        </div>
      </section>
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h2 className="text-xl md:text-2xl font-bold text-gray-900">
                      {allDone ? "You're a Legend!" : "Review Challenge"}
                    </h2>
                    <p className="text-sm text-gray-500">
                      {allDone
                        ? "You completed all 5 reviews. Thank you so much!"
                        : `${progress} of ${PLATFORMS.length} platforms reviewed — keep going!`}
                    </p>
                  </div>
                </div>
              ...
              <div className="flex justify-between mt-3 px-1">
                {PLATFORMS.map((p) => (
                  <div key={p.key} className="flex flex-col items-center gap-1">
                    ...
                    <span className={`text-[10px] font-medium ${completed.has(p.key) ? "text-green-600" : "text-gray-400"}`}>{p.name}</span>
                  </div>
                ))}
              </div>
            </div>
          ...
          <AnimatedSection>
            <div className="text-center mb-8">
              <h2 className="text-3xl md:text-4xl font-bold text-gray-900 mb-4">
                Leave a Review on Every Platform
              </h2>
              <p className="text-xl text-gray-600 max-w-2xl mx-auto">
                Each platform reaches a different audience. A 2-minute review on each one makes a huge difference for our business.
              </p>
            </div>
          </AnimatedSection>
                  ...
                  >
                    ...
                    <div className="flex-grow min-w-0">
                      <div className="flex items-center gap-2 flex-wrap">
                        <span className={`font-bold text-lg ${isDone ? "text-green-700" : "text-gray-900"}`}>{p.name}</span>
                        {p.badge && !isDone && (
                          <span className={`text-xs font-bold px-2.5 py-0.5 rounded-full ${p.badgeColor}`}>
                            {p.badge}
                          </span>
                        )}
                        {isDone && (
                      ...
                      <p className={`text-sm mt-0.5 ${isDone ? "text-green-600" : "text-gray-500"}`}>{p.description}</p>
                    </div>
            ...
            <AnimatedSection>
              <div className="mt-8 text-center p-6 bg-blue-50 rounded-2xl border border-blue-100">
                <p className="text-lg font-semibold text-bubble-primary">
                  {progress === 1 && "Great start! 4 more to go — you're making a real difference."}
                  {progress === 2 && "Awesome! You're almost halfway there. Keep it going!"}
                  {progress === 3 && "More than halfway! Just 2 more platforms to go."}
                  {progress === 4 && "So close! One more review and you've completed them all!"}
                </p>
              </div>
            </AnimatedSection>
          )}
      ...
      {showCelebration && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm" onClick={() => setShowCelebration(false)}>
          <div className="bg-white rounded-3xl p-8 md:p-12 max-w-lg mx-4 text-center shadow-2xl animate-bounce-once" onClick={e => e.stopPropagation()}>
            ...
            <p className="text-lg text-gray-600 mb-6">
              You just helped us on all 5 platforms. We truly appreciate your support — it means the world to our team and helps other families find reliable service.
            </p>
            <div className="flex flex-col sm:flex-row gap-3 justify-center">
              ...
              >
                Get a Friend&apos;s Estimate <ArrowRight className="w-5 h-5" />
              </Link>
              <button
        ...
        <div className="container mx-auto px-4 max-w-4xl relative z-10">
          <AnimatedSection from="bottom">
            <h2 className="text-3xl md:text-4xl font-bold mb-6">
              Need More Soffit &amp; Fascia Work?
            </h2>
            <p className="text-xl text-blue-100 mb-10">
              We&apos;d love to help again. Get a free estimate for your next project.
            </p>
