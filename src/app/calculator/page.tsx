    ...
    const steps = [
        {
            ...
            color: 'text-blue-600',
            bg: 'bg-blue-50',
            title: 'Measure Total Linear Feet',
            desc: 'Walk the outside perimeter of your home with a tape measure and add up the length of every side that has soffit or fascia. Tip: Google Maps "Measure Distance" gives a quick estimate.',
            img: { src: '/images/guide_linear_feet.webp', alt: 'How to measure Linear Feet', caption: 'Add up the length along each side of the roof perimeter. Example: 80 ft front + 60 ft back + 50 ft each side = 240 LF total.' }
        },
        {
            icon: CornerDownRight,
            color: 'text-purple-600',
            bg: 'bg-purple-50',
            title: 'Identify the Overhang Depth',
            desc: 'This is the horizontal distance from the exterior wall out to the very edge of the roof. Measure it from outside. Common sizes: 12" (standard), 16–24" (moderate), 30–36" (deep/custom).',
            img: { src: '/images/guide_overhang.webp', alt: 'How to identify overhang depth', caption: 'Measure from the wall outward to the drip edge of the roof. Different sides of the house may have different overhangs — measure each one.' }
        },
        {
            icon: Hash,
            color: 'text-emerald-600',
            bg: 'bg-emerald-50',
            title: 'Count the Exterior Corners',
            desc: 'Each 90° corner adds extra trim work. A simple box house has 4 corners. An L-shaped house has 6. Mention the count in your quote request so we don\'t underestimate trim labor.',
            img: { src: '/images/guide_corners.webp', alt: 'How to count exterior corners', caption: 'Count every outward-facing corner of the roofline. Rectangle = 4 corners. L-shape = 6. U-shape = 8.' }
        },
    ];
                ...
                >
                    <div className="flex items-center gap-3">
                        ...
                        <div className="text-left">
                            <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest block leading-none mb-0.5">Client Guide</span>
                            <span className="text-sm font-bold text-slate-700">How to measure before using the sliders</span>
                        </div>
                    </div>
                    <svg
                        ...
                        <div className="px-6 pb-6 space-y-4 border-t border-slate-200 pt-4">
                            ...
                            <div className="bg-amber-50 border border-amber-200 rounded-2xl p-4 flex gap-3 items-start">
                                ...
                                <p className="text-xs text-amber-800 font-medium leading-relaxed">
                                    <strong>Not sure about the exact size?</strong> No problem. Use an estimate now — our team corrects it for free during the on-site visit.
                                </p>
                            </div>
                        </div>
                    </div>
...
function SmartCalculatorApp() {
    ...
    const handleRepairSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        try {
            ...
            if (typeof window.fbq === 'function') window.fbq('track', 'Lead', { content_name: 'Calculator Quote' });
    ...
    const jobLabel = projectType === 'new' ? 'New Install'
        : projectType === 'replace_1' ? 'R&R — 1 Story'
            : projectType === 'replace_2' ? 'R&R — 2 Story'
                : '';
    ...
    return (
        <>
            ...
            >
                ...
                {/* Nav Bar */}
                <div className="h-16 bg-gray-50 border-b border-gray-100 flex items-center px-4 sm:px-8 justify-between relative z-20">
                    <div className="flex items-center gap-2">
                        <Sparkles className="w-5 h-5 text-bubble-primary" />
                        <span className="font-bold text-bubble-navy tracking-tight">Bubbles<span className="text-bubble-primary">AI</span> Estimator</span>
                    </div>
                    {view !== 'intro' && (
                        <button
                ...
                {view === 'intro' && (
                    <div className="flex-1 p-10 md:p-16 flex flex-col items-center justify-center text-center animate-in fade-in zoom-in-95 duration-500">
                        ...
                        <h1 className="text-4xl md:text-5xl font-extrabold text-bubble-navy mb-4 tracking-tight">
                            Get Your Estimate Fast.
                        </h1>
                        <p className="text-xl text-gray-500 max-w-lg mb-12">
                            Answer 2 quick questions and get a transparent Soffit &amp; Fascia price — in under a minute.
                        </p>
                        <button
                        ...
                        >
                            <span className="relative z-10">Start My Estimate</span>
                ...
                {view === 'type_select' && (
                    <div className="flex-1 p-8 md:p-12 flex flex-col animate-in slide-in-from-right-8 duration-500">
                        <h2 className="text-3xl font-extrabold text-center text-bubble-navy mb-2">What do you need today?</h2>
                        <p className="text-center text-gray-500 mb-10">Select your project type so we can calculate the right price.</p>

                        <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-5 max-w-4xl mx-auto w-full">
                            {[
                                { type: 'new' as const, label: 'New Install', sub: 'New construction. Clean install, no removal.', icon: Home, tw: 'hover:border-bubble-primary hover:shadow-blue-500/10', ico: 'bg-blue-50 text-bubble-primary' },
                                { type: 'replace_1' as const, label: 'Replace — 1 Story', sub: 'Single-story home. Remove old material and install new.', icon: Settings, tw: 'hover:border-purple-500 hover:shadow-purple-500/10', ico: 'bg-purple-50 text-purple-600' },
                                { type: 'replace_2' as const, label: 'Replace — 2 Story', sub: 'Two-story home. Includes height complexity.', icon: Settings, tw: 'hover:border-orange-400 hover:shadow-orange-500/10', ico: 'bg-orange-50 text-orange-500' },
                            ...
                            <button onClick={() => handleTypeSelect('repair')}
                                ...
                                <h3 className="text-base font-bold text-white mb-1 relative z-10">Repair</h3>
                                <p className="text-sm text-gray-400 relative z-10">Send us a photo. Our team contacts you with a precise quote.</p>
                    ...
                    >
                        ...
                        {repairStatus === 'success' && (
                            <div className="flex-1 p-8 md:p-12 flex flex-col items-center justify-center text-center animate-in fade-in zoom-in-95 duration-500">
                                ...
                                <h2 className="text-3xl font-extrabold text-gray-900 mb-2">Photos Sent!</h2>
                                <p className="text-gray-500 max-w-sm mb-8">Our team received your request and will call you back with a precise repair quote — usually within a few hours.</p>
                                <button type="button" onClick={() => { setView('intro'); setRepairFiles([]); setRepairStatus('idle'); }}
                                    ...
                                    <ArrowRight className="w-4 h-4" /> Back to Estimator
                                </button>
                            </div>
                        )}
                        ...
                        <div className={cn('flex-1 p-8 md:p-12 flex flex-col items-center animate-in fade-in duration-500', (view !== 'ai_repair' || repairStatus === 'success') && 'hidden')}>
                            <div className="text-center mb-8">
                                <div className="inline-flex items-center gap-2 bg-cyan-50 border border-cyan-200 text-cyan-700 text-xs font-bold px-4 py-1.5 rounded-full uppercase tracking-widest mb-4">
                                    <Camera className="w-3.5 h-3.5" /> Step 1 of 2 — Upload Photos
                                </div>
                                <h2 className="text-3xl font-extrabold text-gray-900 mb-2">Show Us the Damage</h2>
                                <p className="text-gray-500 max-w-md mx-auto">Upload photos of the broken or damaged area. Our team reviews every request and calls you back with a precise quote.</p>
                            </div>
                            ...
                            <div className="w-full max-w-xl space-y-5">
                                ...
                                <div className="bg-amber-50 border border-amber-200 rounded-2xl px-5 py-4 flex gap-3 items-start">
                                    ...
                                    <div>
                                        <p className="text-sm font-bold text-amber-900">The more photos, the better your quote.</p>
                                        <p className="text-xs text-amber-700 mt-0.5 leading-relaxed">
                                            Send <strong>at least 2 photos</strong> — different angles help us spot hidden damage and give you a more accurate price. Up to 5 photos accepted.
                                        </p>
                                    </div>
                                </div>
                                    ...
                                    <div className="grid grid-cols-3 gap-3 animate-in fade-in duration-300">
                                        ...
                                        {repairPreviews.length < 5 && (
                                            ...
                                            >
                                                ...
                                                <span className="text-[10px] font-bold text-gray-400 group-hover:text-bubble-primary">Add photo</span>
                                            </div>
                                        )}
                                    </div>
                                )}
                                ...
                                {repairPreviews.length === 0 && (
                                    ...
                                    >
                                        ...
                                        <h3 className="text-xl font-bold text-gray-900 mb-1">Tap to Upload Photos</h3>
                                        <p className="text-gray-400 text-sm text-center max-w-xs">Different angles of the damage — soffit, fascia, roof edge.</p>
                                ...
                                {repairPreviews.length > 0 && (
                                    <div className={cn(
                                        ...
                                        <p className={cn('text-sm font-semibold', repairPreviews.length >= 2 ? 'text-emerald-800' : 'text-amber-800')}>
                                            {repairPreviews.length >= 2
                                                ? `${repairPreviews.length} photos selected — great! You're ready to continue.`
                                                : 'Just 1 photo so far. Add at least one more for a better quote.'
                                            }
                                        </p>
                                    </div>
                                )}
                                ...
                                {repairPreviews.length > 0 && (
                                    ...
                                    >
                                        Continue — Fill Your Info <ArrowRight className="w-5 h-5" />
                                    </button>
                                )}
                            </div>
                        </div>
                        ...
                        <div className={cn('flex-1 p-8 md:p-12 flex flex-col items-center animate-in slide-in-from-right-8 duration-500 overflow-y-auto', (view !== 'repair_contact' || repairStatus === 'success') && 'hidden')}>
                            <div className="w-full max-w-2xl">
                                <div className="text-center mb-7">
                                    <div className="inline-flex items-center gap-2 bg-cyan-50 border border-cyan-200 text-cyan-700 text-xs font-bold px-4 py-1.5 rounded-full uppercase tracking-widest mb-4">
                                        <ArrowRight className="w-3.5 h-3.5" /> Step 2 of 2 — Your Info
                                    </div>
                                    <h2 className="text-3xl font-extrabold text-gray-900 mb-2">Get Your Repair Quote</h2>
                                    <p className="text-gray-500 max-w-md mx-auto">Photos received. Fill in your details and our team will call you back with a precise repair estimate — usually within a few hours.</p>
                                </div>
                                    ...
                                    <div className="bg-gray-50 border border-gray-200 rounded-2xl p-4 mb-6">
                                        <div className="flex items-center justify-between mb-3">
                                            <p className="text-sm font-bold text-gray-800 flex items-center gap-1.5">
                                                ...
                                                {repairPreviews.length} photo{repairPreviews.length > 1 ? 's' : ''} attached
                                            </p>
                                ...
                                <div className="space-y-5">
                                    <div className="grid sm:grid-cols-2 gap-4">
                                        <div>
                                            <label htmlFor="repair-name" className="block text-sm font-bold text-gray-700 mb-1.5">Your Name <span className="text-red-500">*</span></label>
                                            <input id="repair-name" type="text" name="name" required autoComplete="name" placeholder="John Smith"
                                        ...
                                        <div>
                                            <label htmlFor="repair-phone" className="block text-sm font-bold text-gray-700 mb-1.5">Phone <span className="text-red-500">*</span></label>
                                            <input id="repair-phone" type="tel" name="phone" required autoComplete="tel" placeholder="(407) 000-0000"
                                                className="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-bubble-primary focus:border-bubble-primary outline-none transition-all text-sm font-medium text-gray-900" />
                                        </div>
                                    </div>
                                    <div>
                                        <label htmlFor="repair-address" className="block text-sm font-bold text-gray-700 mb-1.5">Property Address <span className="text-red-500">*</span></label>
                                        <input id="repair-address" type="text" name="address" required autoComplete="street-address" placeholder="123 Main St, Orlando, FL"
                                            className="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-bubble-primary focus:border-bubble-primary outline-none transition-all text-sm font-medium text-gray-900" />
                                    </div>
                                    <div>
                                        <label htmlFor="repair-desc" className="block text-sm font-bold text-gray-700 mb-1.5">What happened? (optional)</label>
                                        <textarea id="repair-desc" name="description" rows={3} placeholder="Animal intrusion, storm damage, rotting wood, falling soffit...?"
                                    ...
                                    >
                                        {repairStatus === 'sending'
                                            ...
                                            : <><Camera className="w-5 h-5" /> Send Photos &amp; Get Repair Quote</>
                                        }
                                    </button>
                                    {repairStatus === 'error' && (
                                        <p className="text-center text-sm font-bold text-red-600 bg-red-50 border border-red-200 rounded-xl px-4 py-3">
                                            ⚠ Could not send. Please try again or call us at (407) 715-1790.
                                        </p>
                                    )}
                                    <p className="text-center text-xs text-gray-400 flex items-center justify-center gap-1.5">
                                        <CheckCircle2 className="w-3.5 h-3.5 text-green-500" />
                                        We call back within a few hours · No commitment needed
                                    </p>
                                </div>
                            </div>
                        </div>
                ...
                {view === 'measurements' && (
                    <div className="flex-1 p-8 md:p-12 flex flex-col animate-in slide-in-from-right-8 duration-500 overflow-y-auto">
                        <div className="flex justify-between items-end mb-8 border-b border-gray-100 pb-6">
                            <div>
                                <span className="text-bubble-primary font-bold text-sm tracking-widest uppercase mb-1 block">Step 1 of 1</span>
                                <h2 className="text-3xl font-extrabold text-bubble-navy">Property Details</h2>
                            </div>
                        </div>
                        ...
                        <div className="grid md:grid-cols-2 gap-10 items-start">
                            ...
                            <div className="space-y-8">
                                ...
                                <div>
                                    ...
                                    <div className="flex justify-between text-xs text-gray-400 font-bold mt-2">
                                        <span>Small (50 ft)</span><span>Large (600 ft)</span>
                                    </div>
                                </div>
                                ...
                                <div>
                                    ...
                                    <input id="calc-overhang" type="range" min="6" max="48" step="2"
                                        ...
                                        className="w-full h-2 bg-gray-200 rounded-lg appearance-none cursor-pointer accent-bubble-primary" />
                                    <div className="flex justify-between text-xs text-gray-400 font-bold mt-2">
                                        <span>Standard (12&quot;)</span><span>Deep Custom (48&quot;)</span>
                                    </div>
                                    {overhang > 36 && (
                                        <p className="mt-3 text-xs text-amber-700 bg-amber-50 border border-amber-200 rounded-xl px-4 py-2 font-medium">
                                            ⚠ Overhangs above 36&quot; require an on-site structural review before we can quote online.
                                        </p>
                                    )}
                                </div>
                            </div>
                            ...
                            {/* Right: Material */}
                            <div>
                                <span className="block text-sm font-bold text-gray-700 mb-4">Base Material</span>
                                <div className="space-y-4">
                                    {([
                                        { val: 'aluminum', label: 'Premium Aluminum', sub: 'Lifetime weather resistance. (Recommended)' },
                                        { val: 'vinyl', label: "Builder's Vinyl", sub: 'Cost-effective standard coverage.' },
                        ...
                        <div className="mt-12 flex justify-end">
                            ...
                            >
                                Generate My Quote <Sparkles className="w-5 h-5" />
                            </button>
                        </div>
                    </div>
                )}
                ...
                {view === 'result' && (
                    <div className="flex-1 p-8 md:p-12 flex flex-col justify-center animate-in zoom-in-95 duration-700">
                        <div className="text-center mb-10">
                            ...
                            <h2 className="text-4xl font-extrabold text-gray-900">Your Estimate is Ready.</h2>
                            <p className="text-gray-500 mt-2 font-medium">Transparent pricing for your Orlando property.</p>
                        </div>
                        ...
                        {isBlocked ? (
                            <div className="bg-red-50 p-6 rounded-2xl border border-red-100 flex items-start gap-4">
                                ...
                                <div>
                                    <h3 className="font-bold text-red-900 mb-2">On-Site Review Required</h3>
                                    <p className="text-sm text-red-700">Your overhang depth (&gt;36&quot;) requires structural wood reinforcement before aluminum wrapping. Contact us for a free on-site assessment.</p>
                                </div>
                            </div>
                        ) : (
                            <div className="bg-gray-50 p-8 rounded-3xl border border-gray-200">
                                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-end gap-4 sm:gap-0 mb-6 pb-6 border-b border-gray-200">
                                    <div>
                                        <p className="text-xs font-bold text-gray-400 uppercase tracking-widest mb-2">Total Project Estimate</p>
                                        <p className="text-3xl sm:text-5xl font-extrabold text-bubble-navy tracking-tight">
                                            ${finalTotal.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                                        </p>
                                    </div>
                                    <div className="sm:text-right">
                                        <p className="text-xs font-bold text-gray-400 uppercase tracking-widest mb-1">To Schedule</p>
                                        <p className="text-xl sm:text-2xl font-bold text-emerald-600">
                                            Deposit: ${deposit.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                                        </p>
                                    </div>
                                </div>
                                ...
                                <div className="flex flex-wrap gap-2 mb-4">
                                    ...
                                    <span className="bg-white border border-gray-200 px-3 py-1.5 rounded-lg text-sm font-bold text-gray-700">{overhang}&quot; overhang</span>
                                </div>

                                <p className="text-xs text-gray-400 mb-6 italic">
                                    * Automated estimate based on your inputs. This is not a contract or binding quote. Final price confirmed after our free on-site visit. All services subject to our <Link href="/terms" className="underline hover:text-gray-600">Service Agreement</Link>.
                                </p>

                                <Link
                                    href={`/contact?total=${Math.round(finalTotal)}&type=${projectType}&material=${material}&lf=${linearFeet}&overhang=${overhang}`}
                                ...
                                >
                                    Lock This Quote &amp; Book a Visit <ArrowRight className="w-6 h-6" />
                                </Link>
                            </div>
                        )}
                    </div>
            ...
            <Suspense fallback={
                <div className="w-full max-w-4xl h-[600px] flex items-center justify-center bg-white rounded-[2.5rem] shadow-xl">
                    <div className="flex flex-col items-center gap-4">
                        ...
                        <p className="font-bold text-gray-400 animate-pulse">Loading Estimator...</p>
                    </div>
                </div>
            }>
                <SmartCalculatorApp />
