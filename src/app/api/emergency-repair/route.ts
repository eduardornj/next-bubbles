...
export async function POST(req: NextRequest) {
    try {
        ...
        if (recaptchaToken) {
            try {
                ...
                if (!recaptchaData.success || (recaptchaData.score ?? 0) < 0.5) {
                    console.warn('[emergency-repair] reCAPTCHA validation failed:', { success: recaptchaData.success, score: recaptchaData.score, ip });
                    return NextResponse.json({ error: 'Failed bot verification. Please try again.' }, { status: 403 });
                }
            } catch (err) {
                console.error('[emergency-repair] reCAPTCHA verification error:', err);
        ...
        await transporter.sendMail({
            ...
            subject: `🚨 EMERGENCY — ${esc(name)} · ${esc(damageType)}`,
            html: buildEmailHtml({ name: esc(name), phone: esc(phone), phoneDigits, address: esc(address), gateCode: esc(gateCode), damageType: esc(damageType), description: esc(description), photoCount: validPhotos.length, date }),
            attachments,
        });
        ...
        console.error('[emergency-repair] SMTP_HOST:', process.env.SMTP_HOST, 'Error:', e.message);
